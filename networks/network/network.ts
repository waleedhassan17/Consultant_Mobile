...
// Response interceptor for Therapist API
TherapistAxiosInstance.interceptors.response.use(
  (response) => {
    console.log('✅ Therapist API Response status:', response.status);
    return response;
  },
  async (error) => {
    console.error('❌ Therapist API Response error:', {
      message: error.message,
      code: error.code,
      response: error.response?.data,
      config: error.config ? {
    ...
    if (error.code === "ERR_NETWORK" || error.message === "Network Error") {
      console.error("Therapist API network is down or unreachable");
